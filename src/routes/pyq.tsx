...
function LivePractice() {
  ...
  const load = async () => {
    ...
    try {
      const [m, n] = await Promise.all([fetchMcqs(), fetchMains()]);
      setMcqs(m); setMains(n);
    } catch (e) {
  ...
  const empty = !loading && !error && (mcqs?.length ?? 0) === 0 && (mains?.length ?? 0) === 0;

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <Badge variant="outline" className="border-gold/50 text-gold">Vercel /mcqs · /mains</Badge>
      ...
      {empty && (
        <Card className="shadow-sm">
          <CardContent className="p-6 text-sm text-muted-foreground">
            Backend endpoints returned no items. Prelims MCQs and Mains prompts are generated by the backend's local Ollama analysis flow and aren't populated yet. Use the <strong>PYQ Bank</strong> tab for real past-year questions in the meantime.
          </CardContent>
        </Card>
      )}
